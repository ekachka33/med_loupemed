...
class ContactInfoAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        (None, {"fields": ("name", "short_description", "is_active")}),
        (
            "Контактные данные",
            {
                "fields": (
...
class DoctorAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        (None, {"fields": ("name", "specialty", "photo", "is_active")}),
        (
        ...
        ("Подробная информация", {"fields": ("bio", "experience_years", "education")}),
        (
...
class ServiceAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        (None, {"fields": ("name", "slug", "category", "image", "is_active")}),
        ("Описание", {"fields": ("short_description", "full_description")}),
        ("Цены", {"fields": ("base_price",)}),
        ("Связи", {"fields": ("doctors",)}),
    )
...
class DoctorScheduleAdmin(admin.ModelAdmin):
        ...
        if request.method == "POST":
            ...
            if form.is_valid():
                ...
                for doctor in doctors_to_process:
                    if (
                        not doctor.default_start_time
                        or not doctor.default_end_time
                        or doctor.default_interval_minutes <= 0
                    ):
                    ...
                    while current_date <= end_date:
                        if not DoctorSchedule.objects.filter(
                            doctor=doctor, date=current_date
                        ).exists():
                            try:
                ...
                messages.success(
                    request,
                    f"Успешно сгенерировано {generated_count} записей расписания.",
                )
    ...
    def changelist_view(self, request, extra_context=None):
        ...
        extra_context["generate_schedule_url"] = reverse(
            "admin:core_doctorschedule_generate_schedule"
        )
...
class MedicalRecordAdmin(admin.ModelAdmin):
    ...
    def get_form(self, request, obj=None, **kwargs):
        ...
        if obj is None and hasattr(request.user, "doctor_profile"):
            ...
            form.base_fields["uploaded_by_doctor"].widget.attrs["disabled"] = True
        return form
...
class FAQItemAdmin(admin.ModelAdmin):
    ...
    fieldsets = ((None, {"fields": ("question", "answer", "is_published")}),)
    ...
    def answer_preview(self, obj):
        return (
            obj.answer[:75] + "..."
            if obj.answer and len(obj.answer) > 75
            else obj.answer
        )
