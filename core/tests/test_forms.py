...
from core.models import Doctor, Review, Service, Appointment, DoctorSchedule, FAQItem, MedicalRecord

# Импортируем формы
from core.forms import (
    ReviewForm, ContactForm, AppointmentAdminForm,
    DoctorScheduleAdminForm, UserAppointmentForm, MedicalRecordUploadForm,
    UserQuestionForm
)
...
class ContactFormTest(TestCase):
    ...
    def test_contact_form_valid_data_with_email(self):
        ...
        self.assertEqual(form.cleaned_data['message'], 'Привет, это тестовое сообщение.')
    ...
    def test_contact_form_no_email_or_phone(self):
        ...
        self.assertIn("Пожалуйста, укажите хотя бы свой Email или номер телефона для связи.", form.errors['__all__'][0])
...
class AppointmentAdminFormTest(TestCase):
    ...
    def setUp(self):
        ...
        self.doctor = Doctor.objects.create(name="Др. Смит", specialty="Окулист", is_active=True)
        self.service = Service.objects.create(name="Осмотр глаз", base_price=500, duration_minutes=30)
...
class DoctorScheduleAdminFormTest(TestCase):
    ...
    def setUp(self):
        self.doctor = Doctor.objects.create(name="Др. Джон Доу", specialty="Педиатр", is_active=True)
...
class UserAppointmentFormTest(TestCase):
    ...
    def setUp(self):
        ...
        self.user = User.objects.create_user(username='patient_user', password='password')
        self.doctor1 = Doctor.objects.create(name="Др. Елена", specialty="Стоматолог", is_active=True)
        self.doctor2 = Doctor.objects.create(name="Др. Олег", specialty="Офтальмолог", is_active=False)
        self.service1 = Service.objects.create(name="Чистка зубов", base_price=1000, duration_minutes=60, is_active=True)
        self.service2 = Service.objects.create(name="Удаление зуба", base_price=2000, duration_minutes=45, is_active=False)
    ...
    def test_user_appointment_form_valid_data(self):
        ...
        form_data = {
            ...
            'date': (date.today() + timedelta(days=7)).isoformat(),  # Дата из расписания
    ...
    def test_user_appointment_form_no_time_selected(self):
        ...
        self.assertIn('Пожалуйста, выберите доступное время приема.', form.errors['time'])
...
class MedicalRecordUploadFormTest(TestCase):
    ...
    def setUp(self):
        self.user_patient = User.objects.create_user(username='patient_for_record', password='password')
        self.user_doctor_account = User.objects.create_user(username='doctor_uploader', password='password')
        self.doctor = Doctor.objects.create(
        ...
        self.service = Service.objects.create(name="Общий анализ крови", base_price=300, duration_minutes=15)
        ...
        self.appointment2 = Appointment.objects.create(
            ...
            doctor=self.doctor,
            service=self.service,
            date=date.today() + timedelta(days=2), # Предстоящая запись (статус confirmed)
            time=time(14, 0),
            status='confirmed'
        )
    ...
    def test_medical_record_upload_form_valid_data(self):
        ...
        test_file = SimpleUploadedFile("test_document.pdf", b"This is a test PDF content.")
        ...
        form = MedicalRecordUploadForm(data=form_data, files=form_files, doctor=self.doctor)
        ...
        self.assertEqual(os.path.basename(medical_record.file.name), "test_document.pdf")
