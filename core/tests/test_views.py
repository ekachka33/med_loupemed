...
from core.forms import ContactForm, ReviewForm, UserQuestionForm
...
class SubmitReviewViewTest(BaseViewTest):
    def test_submit_review_success_via_ajax_post(self):
        ...
        response = self.client.post(
            reverse("core:submit_review"), data, HTTP_X_REQUESTED_WITH="XMLHttpRequest"
        )
    ...
    def test_submit_review_invalid_form_via_ajax_post(self):
        ...
        )  # Убедимся, что есть поле 'errors' в JSON

        self.assertEqual(
            Review.objects.count(), initial_review_count
    ...
    def test_submit_review_non_ajax_post(self):
        ...
        self.assertJSONEqual(
            str(response.content, encoding="utf8"), expected_json_response
        )
...
class SubmitFeedbackTest(BaseViewTest):
    ...
    @patch("core.views.send_mail")
    @patch(
        "core.models.ContactRequest.objects.create", side_effect=Exception("DB Error")
    )
    def test_submit_feedback_db_error(self, mock_create, mock_send_mail):
        ...
        self.assertFalse(json_response["success"])
        self.assertIn(
            "Произошла ошибка при сохранении вашего запроса", json_response["message"]
        )
        mock_send_mail.assert_not_called()
...
class FAQPageTest(BaseViewTest):
    ...
    def test_faq_page_context(self):
        ...
        self.assertIn(
            self.faq_item, response.context["faq_items"]
        )  # Проверяем, что опубликованный FAQ есть
...
class SubmitQuestionTest(BaseViewTest):
    def test_submit_question_success(self):
        ...
        self.assertJSONEqual(
            ...
            {
                "success": True,
                "message": "Ваш вопрос успешно отправлен! Мы ответим на него в ближайшее время.",
            },
        )
