...
class ContactForm(forms.Form):
    ...
    phone_number = forms.CharField(
        ...
        required=False,
        label="Ваш телефон",
        help_text="Мы свяжемся с вами по телефону или email.",
    )  # Новое поле
...
class UserAppointmentForm(forms.ModelForm):
    ...
    time = forms.TimeField(
        ...
        widget=forms.Select(
            attrs={"class": "form-control time-slots-select", "disabled": "disabled"}
        ),
        required=False,
    )
...
class MedicalRecordUploadForm(forms.ModelForm):
    ...
    class Meta:
        ...
        widgets = {
            "title": forms.TextInput(
                attrs={"placeholder": "Название документа или анализа"}
            ),
            "notes": forms.Textarea(
                attrs={
                    "rows": 3,
                    "placeholder": "Дополнительные примечания к файлу (например, интерпретация результатов)",
                }
            ),
        }
        labels = {
    ...
    def __init__(self, *args, **kwargs):
        ...
        if doctor:
            existing_appointment_ids = MedicalRecord.objects.values_list(
                "appointment_id", flat=True
            )
            self.fields["appointment"].queryset = (
                Appointment.objects.filter(
                    doctor=doctor, status__in=["completed", "confirmed"]
                )
                .exclude(id__in=existing_appointment_ids)
                .order_by("-date", "-time")
            )
        else:
            self.fields["appointment"].queryset = Appointment.objects.all().order_by(
                "-date", "-time"
            )
    ...
    class Meta:
        ...
        widgets = {
            "question": forms.Textarea(
                attrs={"rows": 4, "placeholder": "Задайте свой вопрос..."}
            ),
        }
        labels = {
            "question": "Ваш вопрос",
