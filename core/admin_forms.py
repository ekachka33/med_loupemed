...
class GenerateScheduleForm(forms.Form):
    ...
    end_date = forms.DateField(
        label="Конечная дата",
        widget=forms.DateInput(attrs={'type': 'date', 'class': 'vDateField'}),
        initial=timezone.now().date() + timezone.timedelta(days=30), # По умолчанию - через месяц
    ...
    def __init__(self, *args, **kwargs):
        ...
        from .models import Doctor # Импортируем Doctor здесь, чтобы избежать циклического импорта
        self.fields['doctors'].queryset = Doctor.objects.filter(is_active=True).order_by('name')
    ...
    def clean(self):
        ...
        if start_date and end_date:
            ...
            if end_date < timezone.now().date():
                raise forms.ValidationError("Нельзя генерировать расписание на прошедшие даты.")
        return cleaned_data